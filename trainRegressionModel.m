...
% Auto-generated by MATLAB on 01-Oct-2025 07:41:48
...
trainedModel.About = 'This struct is a trained model exported from Regression Learner R2024b.';
...
% 
% % サイクルごとの検証MSEの推移をプロット
% numCycles = regressionEnsemble.NumTrained;
% kfold = 5;
% 
% % 交差検証用のインデックスを取得
% cv = cvpartition(height(predictors), 'KFold', kfold);
% 
% msePerCycle = zeros(numCycles, 1);
% 
% for n = 1:numCycles
%     mseFolds = zeros(kfold, 1);
%     for k = 1:kfold
%         testIdx = test(cv, k);
%         trainIdx = training(cv, k);
% 
%         % foldごとにモデルを再学習
%         model = fitrensemble(predictors(trainIdx,:), response(trainIdx), ...
%             'Method', 'LSBoost', ...
%             'NumLearningCycles', n, ... % ←サイクル数を指定
%             'Learners', template, ...
%             'LearnRate', 0.1);
% 
%         % テストデータに対する予測
%         yPred = predict(model, predictors(testIdx,:));
%         yTrue = response(testIdx);
% 
%         mseFolds(k) = mean((yTrue - yPred).^2);
%     end
%     msePerCycle(n) = mean(mseFolds);
% end
% 
% % プロット
% figure;
% plot(1:numCycles, msePerCycle, '-o');
% xlabel('NumLearningCycles');
% ylabel('Cross-Validated MSE');
% title('MSE vs. NumLearningCycles');
% grid on;